...
impl Color {
    fn new(red: i16, green: i16, blue: i16) -> Result<Color, String> {
        if red < 0 || red > 255 {
            return Err(String::from("Wrong red color value"));
        }
        if green < 0 || green > 255 {
            return Err(String::from("Wrong green color value"));
        }
        if blue < 0 || blue > 255 {
            return Err(String::from("Wrong blue color value"));
        }
        Ok(Color {
            red: red as u8,
            green: green as u8,
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            Err(String::from("Expected array containing 3 items"))
        } else {
